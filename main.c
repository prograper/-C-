...
void teacherOPImplement(char *class_id){//    teacherOpImplement()    教师操作具体实现函数
    ...
    while(break1){
        ...
        switch(i){
            ...
            case 11 :   ///修改学生信息
                        temp_head = readAndWrite(temp_head,creat_class);
                        modifyStudentInfo(temp_head,creat_class);
                        deleteList(temp_head);
                        system("pause");
                        system("cls");
                        break;
            ...
            case 15 :   ///修改密码
                        temp_head = readAndWrite(temp_head,creat_class);
                        modifyTeacherPswd(teacher_account);
                        deleteList(temp_head);
                        system("pause");
                        system("cls");
                        break;
            case 17 :   ///修改学生密码
                        ...
                        system("cls");
                        break;
            case 19 :    if(warningDialog("您确定要退出教师端吗？","退出提醒","")){
                            //exit(1);
                            break1 = 0;
                        }
                        break;
...
void studentOpUI(){//    studentOpUI()    学生端登录后页面
    isCursorActive(0);
    gameUI(13);
    printCorlor(9);
    cursorGoto(14,2);
...
void studentOPImplement(int id,char *class_mesg){
    ...
    int times = 0;
    for(; times < 10; times ++){
        class_file[times] = class_mesg[times];
    }
    class_file[10] = '.';
    class_file[11] = 't';
    class_file[12] = 'x';
    ...
    while(break1){
        int i = 0;
        studentOpUI();
        i = choiceReact(32,12,2,12,16,studentOpUI);
        switch(i){
            case 12 : ///查看成绩
                      head = readAndWrite(head,class_file);
                      stu = searchStudentNode(head,id);
                      int score = stu->stu.c_grade + stu->stu.english_grade + stu->stu.math_grade;
                      cursorGoto(12,4);
                      printf("C语言  数学  英语  总分");
                      cursorGoto(12,5);
                      printf("%d      %d    %d    %d  ",stu->stu.c_grade,stu->stu.math_grade,stu->stu.english_grade,score);
                      cursorGoto(1,8);
                      system("pause");
                      system("cls");
                      deleteList(head);
            ...
            case 14 : ///修改密码
                      ...
                      cursorGoto(43,5);
                      scanf("%s",modify_pswd);
                      if(strcmp(modify_pswd,stu->stu.pswd) == 0){
                          printf("请（谨慎）输入新密码：");
                          cursorGoto(22,6);
                          scanf("%s",modify_pswd);
                          isCursorActive(0);
                          strcpy(stu->stu.pswd,modify_pswd);
                          head = writeAndRead(head,class_file);
                          cursorGoto(1,7);
                          printf("修改成功，请记牢你的新密码");
                      }else{
                          cursorGoto(20,5);
                          printf("旧密码输入错误，请检查后再输入");
                      }
                      cursorGoto(1,9);
                      system("pause");
                      system("cls");
                      ...
                      break;
            case 16 : ///退出学生端
                      break1 = warningDialog("你确定要退出学生端吗？","退出提醒","");
                      if(break1){
                          break1 = 0;
                      }else{
                         break1 = 1;
...
/**由于牵扯到很多函数，过于复杂，不再做UI**/
void stuUserInputInterface(){//    stuUserInputInterface()    学生端登录页面
    int stu_id = 0;
    char id_pswd[11];
    char teacher_id[11];
    char class_file[15];
    ...
    while(choice){
        ...
        scanf("%s",teacher_id);
        cursorGoto(35,12);
        //printf("%d %s %s",stu_id,id_pswd,teacher_id);
        cursorGoto(26,12);
        printf("登录扣1，退出扣0 [ ]");
        cursorGoto(44,12);
        scanf("%d",&choice);
        ...
        if(choice){
            ...
            if(file == NULL){
                cursorGoto(26,13);
                printf("班级文件操作失误，登录失败");
                mark1 = 0;
            }else{
                while(fscanf(file,"%d %s %s %d %d %d\n",&temp_id,temp_pswd,temp_name,&temp_c,&temp_m,&temp_e) != EOF){
                    if(temp_id == stu_id){
                        mark = 0;
                        if(strcmp(id_pswd,temp_pswd) == 0){
                            system("cls");
                            Sleep(800);
                            studentOPImplement(stu_id,class_file);
                            choice = 0;
                            break;
                        }else{
                            cursorGoto(26,14);
...
/**学生端登录UI已被简化，后期有空闲时间补上现已废弃**/
void stuUserUI(){

    int i = 0;
    int break1 = 1;
    while(break1){
        i = choiceReact(17,5,3,5,17,stuUserInputInterface);
        switch(i){
        case 5 :
                 cursorGoto(52,5);
                 printf("学号");
       ...
       case 14 :
                 ...
                 break;
       case 17 :
                 warningDialog("你想退出学生端登录吗？","退出登录提醒","");
                 break1 = 0;
                 break;
        }
    }
...
Node* modifyStudentInfo(Node* head ,const char *file_name){//    nodifyStudentInfo()    三个参数分别是头指针，学生id，学生数学成绩
    ...
    if(mark){
        ...
        this_node = readAndWrite(this_node,file_name);
        now = this_node->next;
        printCorlor(7);
        while(now != NULL){
...
void modifyTeacherPswd( const char *account){//    modifyTeacherPswd()    修改教师账号密码
    char temp_pswd[11];
    ...
    if(temp_file == NULL){
        ...
        while(fscanf(temp_file,"%s\t%s\t%s\t%s\n",temp_account,vertify_pswd,useless_ques,useless_answ) != EOF){
            ...
            now->next = temp;
            now = temp;
            strcpy(now->teach.account,temp_account);
            strcpy(now->teach.pswd,vertify_pswd);
            strcpy(now->teach.question,useless_ques);
            strcpy(now->teach.answer,useless_answ);
        }
    }
    fclose(temp_file);
    temp_file = NULL;
    printf("请输入您的旧密码：\n");
    scanf("%s",temp_pswd);
    tNode *current = temp_head->next;
    while(current != NULL){
        //printf("%s %s %s %s\n",current->teach.account,current->teach.pswd,current->teach.question,current->teach.answer);
        if(strcmp(account,current->teach.account) == 0){
            if(strcmp(temp_pswd,current->teach.pswd) == 0){
                printf("请输入您的新密码：\n");
                scanf("%s",temp_pswd);
                strcpy(current->teach.pswd,temp_pswd);
    ...
    isCursorActive(0);
    if(mark){
        temp_file = fopen("Teacher.txt","w");
        if(temp_file == NULL){
            printf("Modify_Pswd_File_Write_Error\n");
            exit(-1);
        }else{
            now = temp_head->next;
            while(now != NULL){
                fprintf(temp_file,"%s\t%s\t%s\t%s\n",now->teach.account,now->teach.pswd,now->teach.question,now->teach.answer);
                now = now->next;
            }
        }
        fclose(temp_file);
        printf("修改完成!请记牢新密码:%s\n",temp_pswd);
    }
...
Node* readAndWrite(Node *head,const char *file_name){//    readAndWrite()    将文件读入链表中
    FILE *file = NULL;
    Node *this_head = head;
    Node *temp = NULL;
